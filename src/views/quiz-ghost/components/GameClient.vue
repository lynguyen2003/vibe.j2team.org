...
const { questions, options, guesses, status, addGuessAsync, nextQuestion, goToQuestion, isUnlocked, currentQ, isFinished, score, isLoading, isGuessing, isError } = useDailyQuiz<ClientQuoteItem, ClientQuoteItem>('clients')
...
<template>
  <div class="space-y-8 max-w-5xl mx-auto font-body text-text-primary animate-fade-up">
    ...
    <div class="flex flex-col items-center gap-2 mb-6">
      <h2 class="font-display text-3xl font-bold text-accent-amber flex items-center gap-3">
        <span class="text-accent-coral">//</span>
        GUESS THE CLIENT
      </h2>
      <p class="text-text-secondary">Đọc vị khách hàng & PM (5 câu/ngày)</p>
      ...
      <div v-if="!isFinished && !isLoading && !isError" class="mt-4 flex items-center gap-2">
        <button v-for="i in 5" :key="i"
          ...
          :class="[
            currentQ === i - 1 ? 'border-accent-amber bg-accent-amber/20 text-accent-amber scale-110' : 'border-border-default bg-bg-deep text-text-dim hover:text-text-primary',
            ...
            !isUnlocked(i - 1) ? 'opacity-30 cursor-not-allowed hover:text-text-dim' : 'cursor-pointer hover:border-accent-amber/50'
          ]"
        >
          {{ i }}
        </button>
    ...
    <div v-else-if="isLoading" class="border border-border-default bg-bg-surface p-12 flex flex-col items-center justify-center animate-pulse">
      <div class="w-10 h-10 border-4 border-accent-amber border-t-transparent rounded-full animate-spin mb-4"></div>
    ...
    <div v-else-if="!isFinished && questions[currentQ]" class="border border-border-default bg-bg-surface p-6 sm:p-8 relative">
      ...
      <!-- The Quote -->
      <div class="mb-10 px-6 py-10 bg-bg-deep border border-border-default relative">
        <span class="absolute top-2 left-4 text-6xl text-accent-amber/10 font-serif leading-none">"</span>
        <h3 class="font-display text-xl sm:text-2xl font-semibold text-center italic relative z-10 mx-6 text-text-primary">
          {{ questions[currentQ]?.quote }}
        </h3>
        <span class="absolute bottom-[-1rem] right-4 text-6xl text-accent-amber/10 font-serif leading-none rotate-180">"</span>
      </div>
      ...
      <div v-if="status[currentQ] === 'playing'" class="flex flex-col sm:flex-row gap-4 mb-4">
        <div class="relative flex-1">
          <AutocompleteInput
            ...
            :disabled="isGuessing"
            :options="options"
            themeClass="focus:border-accent-amber"
            placeholder="Nghĩa thực sự / Hậu quả là gì?..."
          />
        </div>
        <button @click="handleGuess" :disabled="isGuessing" class="px-8 py-3 bg-accent-amber text-bg-deep font-display font-bold hover:bg-white hover:text-accent-amber transition-colors duration-300 disabled:opacity-50 flex items-center justify-center min-w-[120px]">
      ...
      <div class="overflow-x-auto pb-4 custom-scrollbar">
        <div class="inline-flex flex-col gap-3 min-w-[900px] w-full">
          <!-- Header -->
          <div class="grid grid-cols-7 gap-3 text-center text-xs font-display text-text-dim uppercase tracking-widest pb-3 border-b border-border-default">
            <div class="col-span-3">Nghĩa Thực Sự</div>
            <div>Bị Cáo</div>
            <div>Mảng</div>
            <div class="col-span-2">Hậu quả</div>
          </div>
          
          <!-- Rows -->
          <TransitionGroup name="list" tag="div" class="flex flex-col gap-3">
            <div v-for="guess in [...(guesses[currentQ] || [])].reverse()" :key="guess.id" 
              class="grid grid-cols-7 gap-3 text-center text-sm font-medium">
              <div class="col-span-3 p-3 border flex items-center justify-center transition-colors" :class="getMatchColor(guess._comparison?.realMeaning)">
                {{ guess.realMeaning }}
              </div>
              <div class="p-3 border flex items-center justify-center font-bold tracking-wide transition-colors" :class="getMatchColor(guess._comparison?.speaker)">
                {{ guess._comparison?.speaker === 'correct' ? guess.speaker : '???' }}
              </div>
              <div class="p-3 border flex items-center justify-center transition-colors" :class="getMatchColor(guess._comparison?.field)">
                {{ guess._comparison?.field === 'correct' ? guess.field : '???' }}
              </div>
              <div class="col-span-2 p-3 border flex items-center justify-center italic transition-colors" :class="getMatchColor(guess._comparison?.consequence)">
                {{ guess._comparison?.consequence === 'correct' ? guess.consequence : '???' }}
              </div>
            </div>
          </TransitionGroup>

          <div v-if="(guesses[currentQ] || []).length === 0" class="py-8 text-center text-text-dim italic">
            Chưa có dự đoán nào. Thử nhớ xem ý nghĩa thực sự là gì đi!
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="isFinished" class="border border-border-default bg-bg-surface p-8 text-center animate-fade-up">
      <div class="mx-auto w-20 h-20 mb-6 border-4 border-accent-amber flex items-center justify-center rotate-3">
        <span class="text-4xl">👨‍💼</span>
      </div>
      <h3 class="font-display text-4xl font-bold text-text-primary mb-2">TỔNG KẾT CLIENT</h3>
      ...
      <div class="flex justify-center gap-4 mb-8">
        <div class="text-center p-4 border border-border-default bg-bg-deep w-32">
          <div class="font-display text-4xl font-bold text-accent-amber">{{ score }}</div>
          <div class="text-xs text-text-dim tracking-widest uppercase mt-2">Đọc vị đúng</div>
        </div>
        <div class="text-center p-4 border border-border-default bg-bg-deep w-32">
          <div class="font-display text-4xl font-bold text-accent-coral">{{ 5 - score }}</div>
          <div class="text-xs text-text-dim tracking-widest uppercase mt-2">Bị lừa</div>
        </div>
      </div>
      
      <p class="text-text-dim italic text-sm mb-8">Hãy trau dồi thêm kinh nghiệm thực chiến vào ngày mai nhé.</p>

      <RouterLink to="/" class="inline-flex items-center gap-2 px-6 py-3 border border-border-default hover:border-accent-amber bg-bg-deep text-text-primary hover:text-accent-amber transition-colors font-display text-sm tracking-widest font-bold">
