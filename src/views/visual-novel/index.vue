...
<template>
  <div class="min-h-screen bg-bg-deep text-text-primary font-body">
    ...
    >
      <p
        class="font-display text-xs tracking-widest text-accent-amber mb-4 animate-fade-up"
      >
        // VISUAL NOVEL
      </p>
      <h1
      ...
      <button
        class="mt-10 border border-border-default bg-bg-surface px-8 py-3
               font-display text-sm tracking-widest text-text-primary
      ...
      <RouterLink
        to="/"
        class="mt-6 inline-flex items-center gap-2 text-sm text-text-dim
               transition hover:text-text-secondary animate-fade-up animate-delay-5"
      >
        &larr; Về trang chủ
      </RouterLink>
    </div>
    ...
    <div v-else class="h-screen w-screen relative">
      ...
      <RouterLink
        to="/"
        class="absolute top-4 left-4 z-30 border border-border-default/50 bg-bg-deep/70
               px-3 py-1.5 text-xs text-text-dim transition
               hover:border-accent-coral hover:text-text-secondary backdrop-blur-sm"
      >
        &larr; Trang chủ
      </RouterLink>
    </div>
